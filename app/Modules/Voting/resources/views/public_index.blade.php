@php
    ...
    $typeMeta = [
        'individual_award'   => ['icon' => '👤', 'label' => __('Individual award'), 'color' => 'from-brand-600 to-brand-800'],
        'team_award'         => ['icon' => '🛡️', 'label' => __('Team award'),       'color' => 'from-blue-600 to-brand-800'],
        'team_of_the_season' => ['icon' => '⚽', 'label' => __('Team of the Season'),'color' => 'from-emerald-700 to-brand-800'],
    ];
@endphp
<!DOCTYPE html>
    ...
    @if($open->isNotEmpty())
        <section>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                @foreach($open as $c)
                    @php
                        $meta = $typeMeta[$c->type->value] ?? ['icon' => '🗳️', 'label' => '', 'color' => 'from-brand-600 to-brand-800'];
                    ...
                    <a href="{{ route('voting.show', $c->public_token) }}"
                       class="group block rounded-3xl overflow-hidden bg-white border border-ink-200 shadow-sm hover:shadow-brand hover:-translate-y-0.5 transition">
                        <div class="relative p-5 md:p-6 bg-gradient-to-{{ $dir === 'rtl' ? 'l' : 'r' }} {{ $meta['color'] }} text-white">
                            <div class="flex items-start gap-3">
                                <div class="w-12 h-12 rounded-2xl bg-white/20 backdrop-blur flex items-center justify-center text-2xl flex-shrink-0">
                                    {{ $meta['icon'] }}
                                </div>
                                ...
                                <span class="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/90 text-white text-[10px] font-bold px-2.5 py-1 flex-shrink-0">
                        ...
                        <div class="p-5 md:p-6">
                            <div class="grid grid-cols-2 gap-3 text-sm">
                                <div class="rounded-xl bg-brand-50 border border-brand-200 p-3">
                                    <div class="text-[10px] uppercase tracking-wider text-brand-600 font-bold">{{ __('Closes') }}</div>
                                    <div class="font-extrabold text-brand-900 mt-0.5 text-sm">
                                        {{ $c->end_at->diffForHumans(['parts' => 2, 'short' => true]) }}
                                    </div>
                                    <div class="text-[10px] text-ink-500 mt-0.5">{{ $c->end_at->translatedFormat('d M · H:i') }}</div>
                                </div>
                                <div class="rounded-xl bg-ink-50 border border-ink-200 p-3">
                                    <div class="text-[10px] uppercase tracking-wider text-ink-600 font-bold">{{ __('Votes') }}</div>
                                    <div class="font-extrabold text-ink-900 mt-0.5 text-sm">
                                        {{ number_format($c->votes_count) }}
                                        @if($c->max_voters)
                                            <span class="text-ink-400 font-normal">/ {{ number_format($c->max_voters) }}</span>
                                        @endif
                                    </div>
                                    @if($pct !== null)
                            ...
                            <div class="mt-4 inline-flex items-center justify-center w-full rounded-xl bg-brand-600 group-hover:bg-brand-700 text-white px-5 py-3 font-bold shadow-brand transition">
                                🗳️ {{ __('Vote now') }}
                                <span class="ms-2 transition group-hover:translate-x-1 rtl:group-hover:-translate-x-1">
                                    {{ $dir === 'rtl' ? '←' : '→' }}
                                </span>
                            </div>
                        </div>
