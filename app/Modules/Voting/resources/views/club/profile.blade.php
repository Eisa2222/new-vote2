...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="relative rounded-3xl overflow-hidden shadow-xl">
        ...
        <div class="relative p-8 md:p-10 text-center text-white">
            <div class="inline-flex w-20 h-20 rounded-3xl bg-white/15 backdrop-blur items-center justify-center text-5xl mb-3">
                📬
            </div>
        ...
        <form method="post" action="{{ route('voting.club.profile.save', $row->voting_link_token) }}" class="space-y-4">
            ...
            <div>
                <label class="flex items-center gap-2 text-sm font-bold text-ink-900 mb-1.5">
                    <span class="w-6 h-6 rounded-lg bg-brand-50 text-brand-700 flex items-center justify-center text-xs">📱</span>
            ...
            <div>
                <label class="flex items-center gap-2 text-sm font-bold text-ink-900 mb-1.5">
                    <span class="w-6 h-6 rounded-lg bg-brand-50 text-brand-700 flex items-center justify-center text-xs">✉️</span>
                    <span>{{ __('Email') }}</span>
                    <span class="text-xs font-normal text-ink-400">({{ __('optional') }})</span>
                </label>
