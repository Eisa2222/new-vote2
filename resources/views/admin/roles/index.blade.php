    ...
    @foreach($roles as $role)
        ...
        <div class="rounded-3xl bg-white border border-ink-200 shadow-sm p-5 flex flex-col">
            <div class="flex items-start justify-between gap-2">
                ...
                @if($builtIn)
                    <span class="inline-flex items-center rounded-full bg-brand-100 text-brand-700 px-2 py-0.5 text-[10px] font-bold flex-shrink-0">
                        {{ __('Built-in') }}
                    </span>
                @endif
            </div>
            ...
            <div class="mt-3 flex items-center gap-2 text-xs text-ink-500">
                <span>👥 {{ trans_choice('{1} :n user|[2,*] :n users', $role->users_count, ['n' => $role->users_count]) }}</span>
                <span>·</span>
            ...
            <div class="mt-auto pt-4 flex items-center gap-2">
                ...
                @if(!$builtIn)
                    <form method="post" action="{{ route('admin.roles.destroy', $role) }}"
                        ...
                        @csrf @method('DELETE')
                        <button class="rounded-xl border border-rose-300 text-rose-700 hover:bg-rose-50 px-3 py-2 text-sm font-semibold">
                            🗑 {{ __('Delete') }}
                        </button>
                    </form>
                @endif
