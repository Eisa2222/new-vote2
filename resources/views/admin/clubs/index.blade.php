        ...
        <form method="get" class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div class="flex flex-col md:flex-row gap-3 w-full lg:max-w-3xl">
                <input type="text" name="q" value="{{ request('q') }}"
                    placeholder="{{ __('Search by club name') }}..."
                    ...
                    @forelse($clubs as $club)
                        <tr class="border-b last:border-0 hover:bg-slate-50">
                            ...
                            <td class="py-4 text-gray-600">
                                {{ $club->sports->map(fn($s) => $s->localized('name'))->join('، ') }}</td>
                            <td class="py-4">
                                <span
                            ...
                            <td class="py-4">
                                <div class="flex items-center gap-2 flex-wrap">
                                    ...
                                    <form method="post" action="/admin/clubs/{{ $club->id }}/toggle">
                                        @csrf
                                        <button
                                            class="rounded-lg border border-warning-500/50 text-warning-500 hover:bg-warning-500/10 px-3 py-1.5 text-xs font-medium">
                                    ...
                                    <form method="post" action="/admin/clubs/{{ $club->id }}"
                                        ...
                                        @csrf @method('DELETE')
                                        <button
                                            class="rounded-lg border border-danger-500/50 text-danger-600 hover:bg-danger-500/10 px-3 py-1.5 text-xs font-medium">
                                            🗑 {{ __('Delete') }}
                                        </button>
                                    </form>
                                </div>
