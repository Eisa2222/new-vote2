...
@section('content')
    <form method="post" action="{{ route('admin.campaigns.store') }}" class="space-y-6" id="campaignForm">
        ...
        <div class="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                @isset($leagues)
                    <div>
                        <label class="block text-sm font-medium mb-1">{{ __('League') }} <span
                                class="text-gray-400 text-xs">({{ __('optional') }})</span></label>
                        ...
                        <p class="text-xs text-gray-500 mt-1">{{ __('Select a league to restrict candidates to its clubs.') }}
                        </p>
                    </div>
                @endisset
                <div>
            ...
            <div class="mt-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    @foreach ($rules as $rule)
                        ...
                        <label
                            class="relative flex items-start gap-3 rounded-2xl border-2 border-ink-200 bg-white p-4 cursor-pointer transition
                            ...
                            <input type="checkbox" name="{{ $rule['name'] }}" value="1" class="sr-only peer"
                                x-model="on" @checked($rule['checked'])>
    ...
    --}}
        ...
        <div class="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                @foreach ($awards as $i => $award)
                    ...
                    <label
                        class="relative flex flex-col gap-2 rounded-2xl border-2 border-ink-200 bg-white p-4 cursor-pointer transition
                              hover:border-brand-300
                              has-[:checked]:border-brand-500 has-[:checked]:bg-brand-50/40"
                        x-data="{ on: @js($checked) }">
                        <input type="checkbox" name="categories[{{ $i }}][award_type]"
                            value="{{ $award['key'] }}" class="sr-only peer" x-model="on" @checked($checked)>
                        ...
                        <input type="hidden" name="categories[{{ $i }}][title_ar]"
                            value="{{ $award['title_ar'] }}" :disabled="!on">
                        <input type="hidden" name="categories[{{ $i }}][title_en]"
                            value="{{ $award['title_en'] }}" :disabled="!on">
                        <input type="hidden" name="categories[{{ $i }}][position_slot]" value="any"
                            :disabled="!on">
                        <input type="hidden" name="categories[{{ $i }}][required_picks]" value="1"
                            :disabled="!on">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <span class="w-5 h-5 rounded-full border-2 flex items-center justify-center transition"
                                ...
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3 text-white" viewBox="0 0 20 20"
                                    fill="currentColor" :class="on ? 'opacity-100' : 'opacity-0'">
        ...
        <div
            class="sticky bottom-0 bg-white border-t border-ink-200 p-4 rounded-t-3xl shadow-lg flex items-center justify-between gap-2">
            ...
            <button
                class="inline-flex items-center gap-2 rounded-xl bg-brand-600 hover:bg-brand-700 active:bg-brand-800 text-white px-6 py-2.5 text-sm font-semibold shadow-sm transition">
...
<script id="playersData" type="application/json">{!! $playersJson !!}</script>
            ...
            <script>
                    ...
                    function render() {
                        ...
                        if (pool.length === 0) {
                            ...
                            pool.forEach(p => {
                                ...
                                label.className =
                                    'flex items-center gap-3 rounded-lg p-2 cursor-pointer hover:bg-emerald-50 has-[:checked]:bg-emerald-100 has-[:checked]:border-emerald-400 border border-transparent transition';
                                label.innerHTML =
                                    `
                ...
                function saveDraft() {
                    try {
                        localStorage.setItem(DRAFT_KEY, JSON.stringify(serializeCurrent()));
                    } catch (_) {}
