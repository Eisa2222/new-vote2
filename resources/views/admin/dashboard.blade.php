...
@section('content')
    @php
        ...
        $mini = [
            ...
            [
                ...
                'icon' => '⏳',
                'href' => route('admin.campaigns.index') . '?status=pending_approval',
            ],
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-5">
        @foreach ($cards as $card)
            <a href="{{ $card['href'] }}"
                ...
                <div
                    class="mt-4 text-xs text-brand-700 inline-flex items-center gap-1 opacity-0 group-hover:opacity-100 transition">
                    {{ __('View details') }}
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
        @foreach ($mini as $m)
            ...
            <{{ $Tag }} @isset($m['href']) href="{{ $m['href'] }}" @endisset
        ...
        <div class="xl:col-span-2 card">
            <div class="flex items-center justify-between mb-5">
                ...
                <a href="{{ route('admin.campaigns.index') }}"
                    class="text-brand-700 font-semibold hover:underline">{{ __('View all') }}</a>
            </div>
                    ...
                    <tbody>
                        @forelse($recent_campaigns as $campaign)
                            <tr class="border-b border-ink-100 last:border-0 hover:bg-ink-50">
                                ...
                                <td class="py-3 px-2 text-ink-600 whitespace-nowrap">
                                    {{ $campaign->start_at->format('Y-m-d') }} — {{ $campaign->end_at->format('Y-m-d') }}
                                </td>
                                <td class="py-3 px-2">
                                    <span
                                        class="badge {{ $statusClass[$campaign->status?->value] ?? 'bg-ink-100 text-ink-700' }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="py-12 text-center text-ink-400">{{ __('No campaigns yet.') }}
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
