    ...
    <div class="max-w-6xl">
                ...
                <form method="post" action="/admin/settings/general" enctype="multipart/form-data" class="space-y-5">
                    ...
                    <div class="rounded-2xl border border-ink-200 p-4 bg-ink-50/40">
                        ...
                        <div class="flex items-center gap-4 flex-wrap">
                            <div
                                class="w-20 h-20 rounded-2xl bg-white border border-ink-200 flex items-center justify-center overflow-hidden">
                                @if ($logoUrl)
                                    <img src="{{ $logoUrl }}" alt="{{ __('Current logo') }}"
                            ...
                            <div class="flex-1 min-w-[220px]">
                                ...
                                <p class="text-xs text-ink-500 mt-1">
                                    {{ __('PNG, JPEG or WEBP up to 2 MB. Replaces the default FPA wordmark everywhere.') }}
                                </p>
                                @if ($logoUrl)
                                    <label class="mt-2 inline-flex items-center gap-2 text-xs text-rose-600">
                                        <input type="checkbox" name="platform_logo_clear" value="1"
                                            class="rounded border-ink-300 text-rose-600">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label
                                class="block text-sm font-medium mb-1.5">{{ __('Default max voters (optional)') }}</label>
                        ...
                        <div>
                            <label
                                class="block text-sm font-medium mb-1.5">{{ __('Default campaign duration (days)') }}</label>
                            <input type="number" min="1" max="365" name="default_campaign_days"
                                value="{{ old('default_campaign_days', $generalSettings['default_campaign_days']) }}"
                                required class="w-full rounded-xl border border-ink-200 px-4 py-2.5">
                        </div>
                        <div>
                ...
                <div class="flex items-center justify-between mb-4">
                    <div>
                        ...
                        <p class="text-sm text-ink-500 mt-1">
                            {{ __('Add, edit or deactivate sports. Sports with linked clubs cannot be deleted.') }}</p>
                    </div>
                </div>
                            ...
                            <tr>
                                ...
                                <td class="py-3">
                                    <form method="post" action="/admin/settings/sports/{{ $sport->id }}"
                                        class="hidden edit-{{ $sport->id }}">@csrf @method('PUT')</form>
                                ...
                                <td class="py-3">
                                    <span
                                        class="badge badge-{{ $sport->status->value }}">{{ $sport->status->label() }}</span>
                                </td>
                                ...
                                <td class="py-3 text-end">
                                    <form method="post" action="/admin/settings/sports/{{ $sport->id }}"
                                        ...
                                        @csrf @method('DELETE')
                                        <button
                                            class="inline-flex items-center gap-1 rounded-lg border border-danger-500/50 text-danger-600 hover:bg-danger-500/10 px-3 py-1.5 text-xs font-medium transition">
            ...
            <div class="card">
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-ink-100">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="p-8 text-center text-ink-500">
        ...
        <section data-pane="positions" class="tab-pane hidden space-y-4">
            <div class="card">
                ...
                <div class="mt-5 rounded-2xl bg-info-500/5 border border-info-500/30 text-info-500 p-4 text-sm">
                    ℹ️
                    {{ __('Adding a new position requires a code change to maintain domain invariants (TOS formation). If you need an additional position for a non-football sport, contact the development team.') }}
                </div>
            </div>
        </section>
        ...
        <section data-pane="mail" class="tab-pane hidden space-y-4">
            <div class="card space-y-5">
                ...
                @if ($errors->any())
                    <div
                        class="rounded-2xl bg-danger-500/5 border border-danger-500/30 text-danger-600 p-3 text-sm space-y-1">
                ...
                <form method="post" action="{{ route('admin.settings.mail.update') }}"
                    class="grid grid-cols-1 md:grid-cols-2 gap-4" x-data="{ showPwd: false }">
                    @csrf

                    <div
                        class="md:col-span-2 rounded-2xl bg-info-500/5 border border-info-500/30 p-4 text-sm text-info-500">
                        {{ __('Current status') }}:
                        @if ($mailPasswordSet)
                            ✓ {{ __('SMTP credentials are saved and encrypted.') }}
                        @else
                            ℹ️
                            {{ __('No SMTP server configured yet — emails currently use the :driver mailer from .env.', ['driver' => config('mail.default')]) }}
                        @endif
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="mail_port"
                            value="{{ old('mail_port', $mailSettings['mail_port'] ?: '587') }}" min="1"
                            max="65535" required class="field-input">
                    ...
                    <div>
                        <label class="field-label">
                            ...
                            @if ($mailPasswordSet)
                                <span
                                    class="text-ink-400 text-xs font-normal">({{ __('leave empty to keep current') }})</span>
                            @endif
                        </label>
                        <div class="relative">
                            <input name="mail_password" :type="showPwd ? 'text' : 'password'" autocomplete="new-password"
                    ...
                    <div>
                        ...
                        <select name="mail_encryption" class="field-select">
                            @foreach (['tls' => 'STARTTLS (587)', 'ssl' => 'SSL (465)', 'none' => __('None')] as $v => $l)
                                <option value="{{ $v }}" @selected(old('mail_encryption', $mailSettings['mail_encryption'] ?: 'tls') === $v)>{{ $l }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input name="mail_from_name"
                            value="{{ old('mail_from_name', $mailSettings['mail_from_name']) }}" required
                            class="field-input">
                    </div>
                ...
                <form method="post" action="{{ route('admin.settings.sms.update') }}" class="space-y-5">
                    ...
                    <div x-show="driver === 'twilio'" x-cloak class="rounded-2xl border border-ink-200 p-4 space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input name="sms_twilio_sid"
                                    value="{{ old('sms_twilio_sid', $smsSettings['sms_twilio_sid']) }}"
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input name="sms_twilio_token" :type="showT ? 'text' : 'password'"
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input name="sms_twilio_from"
                                    value="{{ old('sms_twilio_from', $smsSettings['sms_twilio_from']) }}"
                                    placeholder="+19715551234" class="field-input max-w-sm">
                                <p class="field-help">
                                    {{ __('A Twilio-verified sender number including the country code.') }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div x-show="driver === 'unifonic'" x-cloak class="rounded-2xl border border-ink-200 p-4 space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <div class="relative">
                                    <input name="sms_unifonic_appsid" :type="showU ? 'text' : 'password'"
                                        autocomplete="new-password"
                            ...
                            <div>
                                ...
                                <input name="sms_unifonic_sender"
                                    value="{{ old('sms_unifonic_sender', $smsSettings['sms_unifonic_sender']) }}"
                                    placeholder="SFPA" class="field-input">
                                <p class="field-help">
                                    {{ __('Alphanumeric sender name approved by Unifonic (max 11 chars).') }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="rounded-2xl border border-ink-200 bg-ink-50/40 p-4 space-y-3">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            ...
                            <input name="test_message" type="text" maxlength="300"
                                value="{{ old('test_message') }}" placeholder="{{ __('SMS test from SFPA Voting') }}"
                                class="field-input">
                        </div>
                        <p class="field-help">
