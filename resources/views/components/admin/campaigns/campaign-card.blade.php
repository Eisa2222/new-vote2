...
@php
    ...
    $typeLabel =
        $typeLabels[$campaign->type?->value] ?? ucfirst(str_replace('_', ' ', (string) $campaign->type?->value));
    ...
    <div class="mt-6 pt-4 border-t border-ink-100 flex flex-wrap items-center gap-2">
        @if (in_array($status, ['draft', 'rejected'], true))
            ...
            <form method="post" action="{{ route('admin.campaigns.submit-approval', $campaign) }}">
                @csrf
                <button
                    class="inline-flex items-center gap-1.5 rounded-xl bg-brand-600 hover:bg-brand-700 text-white px-4 py-2 text-sm font-semibold shadow-sm transition">
